import { createBannerImagePresignedPutUrl } from "@/lib/r2";
import {
  errorResponse,
  handleRouteError,
  readJsonBody,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!parsed.success) {
      ...
      if (field === "fileSize") {
        return errorResponse("Image exceeds the maximum allowed size", 400, {
          fileSize: [`Maximum file size is ${MAX_BANNER_IMAGE_SIZE_BYTES} bytes`],
        });
      }
    ...
    const upload = await createBannerImagePresignedPutUrl(parsed.data.fileType);
    ...
    return handleRouteError(error, {
      route: "/api/uploads/banners/presign",
      method: "POST",
    });
  }
}
